...
{ //access the property vector
    return owned_place_id_list;
}

void Player::mousePressEvent(QGraphicsSceneMouseEvent *event)
{ //show the player information when mouse pressed
...
{ //set the format of player information when showed
...
{ //player buy the property

    if (target->getPropertyPrice() > money)
    { //check whether the player has enough money to buy the property
        return false;
    }
...
{ //access the player current position
    return position;
}
...
{ //use operator to add money from player
    money += a;
    return *this;
}

Player &Player::operator-=(int a)
{ //use operator to subtract money from player
    money -= a;
    return *this;
}
...
{ //when the player go into jail
    inJail = state;
}

void Player::setmoney(int a)
{ //set the player money directly
    money = a;
}

void Player::setNumOfRestaurant(unsigned int r)
{ //change the number of restaurant
    numOfRestaurant = r;
}
...
{ //the principle of jail
    ...
    if (num != 6)
    {
        ...
        {                      //if player cannot roll a six in two round
            ++jailTurnCounter; //stored how many round player can't roll a six
            jailMessage->setText("Fail to Roll a SIX, \n\nYou still in jail");
        }
        else
        {                 //if player have already roll dice for three round and can't get a six in round three
...
{ //check whether the player is losed
    return losed;
}

void Player::setLosed()
{ //set when player is losed
    losed = true;
}
